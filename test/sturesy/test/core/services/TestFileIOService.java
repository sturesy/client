...
public class TestFileIOService
{
    ...
    @Test
    public void testWriteToFileWithFileWriterSuccess() throws IOException
    {
        FileWriter fileWriter = mock(FileWriter.class);
        doNothing().when(fileWriter).write(eq("abc"));

        FileIOService service = new FileIOService();
        boolean isWritten = service.writeToFile(fileWriter, "abc");

        verify(fileWriter, times(1)).write(eq("abc"));
        verify(fileWriter, times(1)).close();
        assertTrue(isWritten);
    }

    @Test(expected = IOException.class)
    public void testWriteToFileWithFileWriterException() throws IOException
    {
        FileWriter fileWriter = mock(FileWriter.class);
        doThrow(new IOException()).when(fileWriter).write(eq("abc"));
    ...
    public void testWriteToFileWithFileWriterIsNullException() throws IOException
    {
        ...
        verify(fileWriter, times(1)).write(eq("abc"));
        verify(fileWriter, times(0)).close();
        assertFalse(isWritten);
    }

    @Test(expected = IOException.class)
    public void testWriteToFileWithFile() throws IOException
    {
        LoadTestFilesService testFileService = new LoadTestFilesService();
        File file = testFileService.retrieveNotACSVFile();
        FileIOService service = new FileIOService();
        FileIOService spy = spy(service);
        doThrow(new IOException()).when(spy).writeToFile(Mockito.<FileWriter> any(), eq("abc"));
        boolean isWritten = spy.writeToFile(file, "abc");

        verify(spy, times(1)).writeToFile(Mockito.<FileWriter> any(), eq("abc"));
        assertFalse(isWritten);
    }

    @Test
    public void testWriteToFileWithSuccess() throws IOException
    {
        LoadTestFilesService testFileService = new LoadTestFilesService();
        File file = testFileService.retrieveNotACSVFile();
        FileIOService service = new FileIOService();
        FileIOService spy = spy(service);
        doReturn(true).when(spy).writeToFile(Mockito.<FileWriter> any(), eq("abc"));
        boolean isWritten = spy.writeToFile(file, "abc");
