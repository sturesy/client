...
public class TestVotingAnalysisCRUDService
{
    ...
    public void testSaveVotingResultVotesExistFileExist() throws IOException
    {
        ...
        assertNull(result);
        verify(_fileService, times(1)).createCSVFileIfNotExist(Mockito.<File> any());
        verify(_fileService, times(1)).writeToFile(file, "2,B,1000\n1,B,1000\n");
    }
    ...
    public void testSaveVotingResultFileServiceThrowsException() throws IOException
    {
        createCRUDService();
        File file = mock(File.class);
        when(_fileService.createCSVFileIfNotExist(file)).thenThrow(new IOException());
        Set<Vote> votes = createFilledVoteSet();
        String result = _crudService.saveVotingResult(votes, file);

        assertEquals(VotingAnalysisCRUDService.ERROR_EXPORTING_TO_CSV, result);
